...
abstract class EntityController<T>(val entityName:String) {
    ...
    open fun list(
            ...
            @RequestParam("order") order:String?):List<T> {
        ...
        return query.resultList.map { it as T }
    }

    @GetMapping("/item/{id}")
    open fun getItem(@PathVariable("id") itemId:String):Any {
        if (itemId === "new") return ""
        return entityManager.find(Class.forName("ru.itport.yourspendings.entity.$entityName"),getItemId(itemId))
    }
